<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" v-show="showSearch" :inline="true">
      ...
      <el-form-item>
        <el-button type="primary" icon="el-icon-search" size="mini" @click="handleQuery" v-hasPermi="['system:role:query']">搜索</el-button>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      var removeids = [];
      if(row.roleId){
        removeids.push(row.roleId)
      }else{
        removeids = this.ids
      }
      this.$modal.confirm('是否确认删除角色编号为"' + roleIds + '"的数据项？').then(function() {
        return delRole(removeids);
      }).then(() => {
        this.getList();
